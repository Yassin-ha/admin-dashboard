...
const Sidebar = ({ sidebarOpen, setSidebarOpen }: SidebarProps) => {
  ...
  return (
    <aside
      ref={sidebar}
      className={`fixed lg:static inset-y-0 left-0 z-9999 flex h-screen w-72.5 flex-col overflow-y-hidden bg-black transition-transform duration-300 ease-linear dark:bg-boxdark lg:translate-x-0 ${
        ...
        <nav className="mt-5 py-4 px-4 lg:mt-9 lg:px-6">
          ...
          <div>
            ...
            <ul className="mb-6 flex flex-col gap-1.5">
              ...
              {/* <!-- Menu Item Tables --> */}
              {/* <!-- Menu Item special cases --> */}
              <li>
                <NavLink
                  to="/specialCases"
                  className={`group relative flex items-center gap-2.5 rounded-sm py-2 px-4 font-medium text-bodydark1 duration-300 ease-in-out hover:bg-graydark dark:hover:bg-meta-4 ${
                    pathname.includes('specialCases') && 'bg-graydark dark:bg-meta-4'
                  }`}
                >
                  <svg
                    className="fill-current"
